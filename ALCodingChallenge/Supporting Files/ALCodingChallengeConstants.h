//
//  ALCodingChallengeConstants.h
//  ALCodingChallenge
//
//  Created by Alan Leatherman on 11/2/15.
//  Copyright © 2015 Alan Leatherman. All rights reserved.
//

#define kMixSetNavigationBackgroundColor [UIColor colorWithRed:0.0f green:0.0f blue:0.0f alpha:0.9f]
#define kDefaultMixSetNavigationBackgroundViewColor [UIColor colorWithRed:1.0f green:1.0f blue:1.0f alpha:1.f]
#define kMixSetNavigationTextColor [UIColor colorWithRed:1.0f green:1.0f blue:1.0f alpha:1.f]
#define kMixSetNavigationBarBackgroundColorAnimationDuration 0.2f
#define kLongPressDuration .2f
#define kDefaultBlurEffectViewConstraint 55.f
#define kLoadPagePercentToScroll 0.3f
#define kLongPressCenterCellMaxScale 1.5f
#define kShortPressCenterCellMaxScale 1.2f
#define kShortPressCenterCellAnimationDuration 0.3f
#define kUserImageSmallScreenBottomConstraint 45.f
#define kMixDetailUserImageYTranslation -70.f
#define kMixDetailPlayAnimationDuration 0.2f
#define kMixDetailDefaultAnimationDuration 1.f

#define kSpringDefaultDamping .8f
#define kSpringDefaultInitialVelocity .8f
#define kAnimationDefaultDuration 0.5f
#define kAnimationDefaultDelay 0.5f

#define kZoomTransitionDuration 0.3f

extern NSString * const mixesURLString;
extern NSString * const baseURLString;
extern NSString * const paginationMixesURLString;
